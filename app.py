...
def main() -> None:
    ...
    with st.sidebar:
        st.subheader("Style guide fusionné")
        st.json(style_guide.__dict__, expanded=False)

    tab_seg, tab_ocr, tab_glossary, tab_translate, tab_qa = st.tabs(
        [
            "Segmentation",
            "OCR images",
    ...
    with tab_seg:
        ...
        if Path(paths["segments_json"]).exists():
            segs = storage.read_json(paths["segments_json"])
            st.write(f"Segments existants: **{len(segs)}**")
            preview_cols = ["id", "tag", "text"]
            st.dataframe(pd.DataFrame(segs)[preview_cols].head(15), use_container_width=True)
    ...
    with tab_ocr:
        ...
        if Path(paths.get("images_ocr_json", "")).exists():
            ocr_preview = storage.read_json(paths["images_ocr_json"])[:10]
            st.dataframe(pd.DataFrame(ocr_preview), use_container_width=True)

    with tab_glossary:
        st.subheader("3) Glossaire & terminologie")
        gcfg = cfg.get("glossary", {})
        if st.button("Extraire candidats glossaire"):
            segs = storage.read_json(paths["segments_json"])
            glossary_rows, candidates = extract_glossary_candidates(
                segs,
                ...
                max_contexts=gcfg.get("max_contexts", 5),
                return_candidates=True,
            )
            storage.write_json(paths["glossary_json"], glossary_rows)
            candidates_path = paths.get("term_candidates_json", "data/segments/term_candidates.json")
            storage.write_json(candidates_path, [c.as_dict() for c in candidates])
            st.session_state["term_candidates"] = [c.as_dict() for c in candidates]
            st.success(
                f"Glossaire écrit → {paths['glossary_json']} (candidats: {len(glossary_rows)})"
            )

        if Path(paths.get("glossary_json", "")).exists():
            glossary_rows = storage.read_json(paths["glossary_json"])
            st.write(f"Glossaire prêt: **{len(glossary_rows)} entrées**")
            st.dataframe(pd.DataFrame(glossary_rows).head(30), use_container_width=True)

        proposals_cfg = gcfg.get("proposals", {})
        if st.button("Proposer traductions de glossaire"):
            source_candidates = (
                st.session_state.get("term_candidates")
                or (storage.read_json(paths.get("term_candidates_json")) if Path(paths.get("term_candidates_json", "")).exists() else [])
            )
            if not source_candidates:
                st.warning("Aucun candidat trouvé : extrayez d'abord le glossaire.")
            else:
                candidates = [
                    TermCandidate(
                        term=row.get("term", ""),
                        ngram=row.get("ngram", len(row.get("term", "").split())),
                        count=row.get("count", 1),
                        contexts=row.get("contexts", []),
                        signals=row.get("signals", {}),
                        domain_score=row.get("domain_score", 0.0),
                    )
                ...
                proposals_path = paths.get("glossary_proposals_json", "data/segments/glossary_proposals.json")
                storage.write_json(proposals_path, proposals)
                st.success(f"Propositions générées → {proposals_path}")
                st.dataframe(pd.DataFrame(proposals).head(20), use_container_width=True)
            ...
            if st.button("Traduire segments"):
                ...
                for seg in translated:
                    ...
                    if post.get("apply_glossary", True) and glossary_map:
                        ...
                        seg["text_translated"] = BeautifulSoup(
                            f"<div>{tr_html}</div>", "html.parser"
                        ).get_text(" ", strip=True)
                    ...
                    if post.get("verify_numbers_dates", True):
                        warnings = numbers_and_citations_check(
                            seg.get("text",""), seg.get("text_translated","")
                        )
